...
def last_hidden_repe(model,prompt_tokens,layer_id,v,coeff):
    ...
    wrapped_model.reset()
    return(hidden_state_controlled)

def last_hidden(model,prompt_tokens):
    with torch.no_grad():
        output = model(prompt_tokens.to(model.device),output_hidden_states=True)
...
for i in range(-1,-20,-1): #layers
    ...
    coeff = 0.0
    coeff1 = 1.0
    #hidden_state = last_hidden(model,prompt_tokens).to(dtype=torch.float32)
    hidden_state = last_hidden_repe(model,prompt_tokens,layer_id,v,coeff).to(dtype=torch.float32)
    hidden_state_controlled1 = last_hidden_repe(model,prompt_tokens,layer_id,v,coeff1).to(dtype=torch.float32)
    delta_h1 = hidden_state_controlled1 - hidden_state
    delta_h1_norm = torch.norm(delta_h1).item()
    ...
    residual = []
    residual_ratio = []
    for coeff2 in range(1,30):

        hidden_state_controlled2 = last_hidden_repe(model, prompt_tokens, layer_id, v, coeff2).to(dtype=torch.float32)
        delta_h2 = hidden_state_controlled2 - hidden_state
        delta_h2_norm = torch.norm(delta_h2).item()
        scale.append(delta_h2_norm/delta_h1_norm)
        abs_scale.append(delta_h2_norm)
        cos_theta.append((torch.sum(delta_h1*delta_h2)/(delta_h1_norm*delta_h2_norm)).item())
        residual.append(torch.norm(delta_h2 - (coeff2/coeff1)*delta_h1).item())
        residual_ratio.append(torch.norm(delta_h2 - (delta_h2_norm/delta_h1_norm)*delta_h1).item() / torch.norm(hidden_state_controlled2).item())
